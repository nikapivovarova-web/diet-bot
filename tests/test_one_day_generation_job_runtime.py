...
@pytest.mark.anyio
async def test_worker_invalid_local_photo_falls_back_to_text_and_marks_delivered(tmp_path) -> None:
    store = _InMemoryOneDayGenerationJobStore(default_quota=1)
    runtime = OneDayGenerationJobRuntime(store)
    admitted = runtime.admit_queued(
        chat_id=93,
        idempotency_key="invalid-local-photo-fallback",
        request_snapshot=_snapshot(93),
    ).job
    bot = _PhotoFailureBot()
    worker = OneDayGenerationWorker(
        runtime,
        _MealCardProcessor(bot, image_url=str(tmp_path)),
        OneDayGenerationWorkerSettings(worker_id="worker-a", concurrency=1, max_attempts=1),
    )
...
@pytest.mark.anyio
async def test_worker_invalid_local_photo_text_fallback_failure_requires_manual_review(tmp_path) -> None:
    store = _InMemoryOneDayGenerationJobStore(default_quota=1)
    runtime = OneDayGenerationJobRuntime(store)
    admitted = runtime.admit_queued(
        chat_id=94,
        idempotency_key="invalid-local-photo-text-failure",
        request_snapshot=_snapshot(94),
    ).job
    bot = _PhotoFailureBot(fail_messages=True)
    worker = OneDayGenerationWorker(
        runtime,
...
class _MealCardProcessor:
    def __init__(self, bot: _PhotoFailureBot, *, image_url: str = "https://example.com/photo.jpg") -> None:
        self.bot = bot
